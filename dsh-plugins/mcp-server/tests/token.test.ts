...
describe('token file persistence', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'mcp-server-token-'))
    file = join(dir, 'nested', 'mcp-server-token')
  })
...
describe('resolveToken', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'mcp-server-resolve-'))
    file = join(dir, 'mcp-server-token')
  })
