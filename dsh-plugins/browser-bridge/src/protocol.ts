/**
 * Wire contract between the DSH browser-bridge plugin and the browser
 * extension.
 *
 * Zero-dependency module (pure types, constants, and a parser). This exact
 * file is duplicated at `extension/src/protocol.ts` because the extension is
 * a separately-bundled MV3 artifact that cannot import this npm package's
 * source directly (see that file's header comment) — keep the two in sync.
 *
 ...
 * (`packages/browser/bridge-browser/src/protocol.ts`), unchanged.
 *
 * @module
 */
