 ...
 * It is NOT the /api trust fence; it is the bridge path's own auth because the
 * bridge route lives outside the fence by design. The model never sees this
 * token or supplies it as a tool argument — it is resolved once at plugin
 * `apply()` from config or the persisted file.
 *
 * Ported unchanged (behaviorally) from github.com/Lum1104/dsh-browser
 * (`packages/browser/bridge-browser/src/token.ts`).
 *
 * @module
 */
