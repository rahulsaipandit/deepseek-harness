 ...
 * 500ms, x2 per attempt, capped at 10s, jittered 0.5-1x.
 *
 * Ported unchanged from github.com/Lum1104/dsh-browser
 * (`extensions/dsh-browser/src/background/bridge.ts`). The only change is the
 * import path: this file imports the LOCAL copy of `protocol.ts`
 * (`../protocol.ts`) instead of `@deepseek-ai/dsh-bridge-browser/src/protocol.ts`,
 * because this extension is a separately-bundled MV3 artifact that cannot
 * import the host npm package's source directly — see `../protocol.ts`'s
 * header comment.
 *
 * @module
 */

import type { BridgeCaps, ClientFrame, ServerFrame } from '../protocol.ts'
import {
