 ...
 * (`extensions/dsh-browser/src/content/actions.ts`).
 *
 * @module
 */
...
function snapshotAction(args: Record<string, unknown>, ctx: ActionContext): ActionResult {
  ...
  // The baseline updates after every snapshot, so a later delta call (whether
  // or not it too is delta) can always compare against the previous state.
...
async function historyAction(delta: 1 | -1): Promise<ActionResult> {
  resetDeltaState()
  // Same as navigate: answer before navigating (document unload destroys the response port).
