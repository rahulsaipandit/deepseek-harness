 ...
 * (`extensions/dsh-browser/src/content/snapshot.ts`). Adapted: upstream's
 * `region` handling and multi-frame headings are unchanged, but this port
 * only ever builds a single top-level-frame snapshot — see the README
 * "Trust and limitations" section on the dropped iframe-scoped snapshot
 * support.
 *
 * @module
 */
...
export interface SnapshotView {
  ...
  /** total render budget (for capping the rendered output). */
  budgetChars: number
}
...
export function buildSnapshot(ids: ElementIds, options: SnapshotOptions, last: SnapshotView | null): SnapshotView {
  ...
  if (options.delta === true && last !== null) {
    if (last.main !== main.text || last.url !== location.href || last.title !== document.title) {
      changed.push(-1) // -1 = main/title/URL changed (explained at render time)
    }
