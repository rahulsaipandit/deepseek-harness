 ...
 *
 * SECURITY: this listener is `chrome.runtime.onMessage`, reachable ONLY from
 * this extension's own background service worker — never from
 * `window.postMessage`/`onMessageExternal`, which arbitrary web pages could
 * call. There is no such listener anywhere in this file or this extension
 * (verified by grep across `src/`, matching upstream's own verified
 * property).
 *
 * Ported unchanged (behaviorally) from github.com/Lum1104/dsh-browser
 * (`extensions/dsh-browser/src/content/index.ts`). Only the import path for
 * `DEFAULT_SNAPSHOT_MAX_CHARS` changed (local `../protocol.ts` copy instead
 * of the host npm package import — see that file's header comment).
 *
 * @module
 */

import { DEFAULT_SNAPSHOT_MAX_CHARS } from '../protocol.ts'
...
const CONTENT_SCRIPT_LISTENER = '__dshBrowserBridgeContentScriptListener__'
...
function onMessage(message: unknown, _sender: chrome.runtime.MessageSender, sendResponse: (response: ToolResult) => void): true | undefined {
  ...
  if (msg.type === 'DSH_BUDGET') {
    ...
    if (incoming !== undefined) {
      ...
      sendResponse({ ok: true, result: { text: `Snapshot budget updated: ${JSON.stringify(budget)}` } })
    }
    return
  }
