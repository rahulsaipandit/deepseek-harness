...
- 连接时：插件激活会等待 `listTools()`，并在本次组合开始第一个轮次之前，以其公开名称通过 `ctx.tools.register()` 注册每个工具。初始连接、发现或注册失败都会被记录；`failOnStartupError` 为 true 时会拒绝激活，否则以无工具状态激活。
- 监听 `notifications/tools/list_changed` → 触发重新同步；获取阶段失败会保留上一个世代已注册的工具，而注册冲突会回滚本次尝试的世代，导致该服务器暂无任何工具。
- 工具执行：`client.callTool({ name: rawName, arguments }, { signal })`，带超时与中止支持（默认 60 秒，可像其他工具调用一样取消）——公开名称永远不会发送给服务器。
- 规范成功结果为 `{ content: JsonValue[], structuredContent? }`；完整的 MCP JSON 块会为程序化调用方保留。已声明且受支持的 `outputSchema` 会校验 `structuredContent`；不受支持的 schema 词汇会回退为不受约束的 `JsonValue`。
- Native/模型渲染沿用现有的文本投影：文本块以换行符拼接，图片、音频、资源及不受支持的块则变为占位符。如果服务器报告错误，调用会明确失败——模型不会看到虚假的成功。
- 断开/崩溃时：监督者会以指数退避（`reconnect.initialDelayMs` 逐次翻倍，直至 `reconnect.maxDelayMs`）重启原始服务器配置，并在成功后重新执行发现——恢复的世代会替换上一个世代，因此工具既不会重复也不会泄漏。故障期间，上一个正常世代继续保持注册；对其发起的调用会失败，直至恢复。
- 重连按每次故障计入预算：连续失败达到 `reconnect.maxAttempts` 次后，该服务器的工具会被注销，重连停止，直到 HMR 重载或 Host 重启。连接在存活超过 `maxDelayMs` 后会重置该预算，因此偶尔崩溃的服务器可以无限期恢复，而持续崩溃循环的服务器——即使期间有过短暂的成功连接——仍会耗尽预算而不是无限重启。
- 重连状态会在日志中对用户可见：重连中（warn，附带尝试次数与延迟）、已恢复（info）、最终失败与因禁用而丢失（error）。dispose（资源释放）会取消任何待处理的重连。当 `reconnect.enabled: false` 时，连接丢失后工具仍保持已注册状态但调用会失败，直到重新加载——即手动恢复行为。
- 声明零工具的服务器（例如纯资源型连接器）根本不会挂接 `tools/list` 处理器；这种特定的 `McpError`/`MethodNotFound` 响应会被视为空工具列表，而不是连接失败。`tools/list` 的其他错误仍会照常导致本次尝试失败。

## 资源（按需）

服务器声明 `resources` 能力时，每个连接世代会额外注册两个合成工具：`mcp__<serverName>__list_resources`（无参数，列出所有资源及其 name/description/mimeType）和 `mcp__<serverName>__read_resource`（`{ uri }`，返回该资源的文本内容）。两者都会在请求时调用实时客户端——不存在类似 `syncTools` 的重新同步/世代比对机制，因为资源类*工具*在每个连接内是静态的，始终反映上游当前实际存在的资源。没有 `resources` 能力的服务器不会获得这两个工具中的任何一个。参见 `src/resources.ts`。
...
- **Prompts 没有 harness 消费机制，暂缓实现。** Resources 已被桥接（见上文），通过每个具备该能力的服务器提供的两个按需工具实现。
