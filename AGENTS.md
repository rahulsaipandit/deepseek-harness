...
- **`ctx.userQuestions` admits only one registered provider per context** (a second `registerProvider()` throws `DUPLICATE_PROVIDER`); `packages/host/apiproxy` already holds that slot whenever a UI is attached, so a plugin sharing the seam must join its existing relay, not compete for the slot. `ctx.approval` has no such limit — `'approval/request'` is a scope-filtered Cordis waterfall, so composed answerers (a UI's and a plugin's) coexist by design ([worked example](docs/adr/rp_dshPlugin_imChat.md#3-approvalquestion-relay)).
